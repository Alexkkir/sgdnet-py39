...
def preprocess_imagesandsaliencyforiqa(imgpaths1, simgpaths1, shape_r, shape_c, crop_h=224, crop_w=224, mirror=False):
    ...
    for patha, pathb in zip(imgpaths1, simgpaths1):
        ...
        image = np.asarray(
            image[h_off: h_off+crop_h, w_off: w_off+crop_w], np.float32)
        simage = simage[h_off: h_off+crop_h, w_off: w_off+crop_w]
        newimg_h = int(
            int(int(int(int(crop_h/2.0+0.5)/2.0+0.5)/2.0+0.5)/2.0+0.5)/2.0+0.5)
        newimg_w = int(
            int(int(int(int(crop_w/2.0+0.5)/2.0+0.5)/2.0+0.5)/2.0+0.5)/2.0+0.5)
