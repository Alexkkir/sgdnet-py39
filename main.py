...
def evaluationmetrics(_y, _y_pred):
    ...
    # srocc is not always accurate. It need using matlab code to compute.
    srocc = stats.spearmanr(q, sq)[0]
...
class DataGenerator(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        images = [os.path.join(im_dir, im_names[idx])
                  for idx in range(len(index))]
        # images = [os.path.join(test_im_dir, im_names[idx]) for idx in range(len(index))]
        simages = [os.path.join(sim_dir, im_names[idx])
                   for idx in range(len(index))]
        maps = [mos[idx] for idx in range(len(index))]
        X, X2 = preprocess_imagesandsaliencyforiqa(images[0:len(index)], simages[0:len(
            index)], shape_r, shape_c, mirror=self.mirror, crop_h=shape_r, crop_w=shape_c)
...
if __name__ == '__main__':
    ...
    log_dir = './data' + \
        '/EXP{}-{}-testtxt/'.format(args.exp_id, args.database)

    b_s = args.batch_size
    crop_h = config['shape_r']
    # print (crop_h) #384
    crop_w = config['shape_c']
    model = SGDNet(basemodel=args.basemodel, saliency=args.saliency, CA=args.CA,
                   fixed=args.fixed, img_cols=crop_w, img_rows=crop_h, out2dim=args.out2dim)

    print("Compile SGDNet Model")
    opt = Adam(lr=args.lr)
    alpha = args.alpha
    if args.saliency == 'output':
        model.compile(optimizer=opt, loss=['mae', TVdist], loss_weights=[
                      1.0 / (1 + alpha), alpha / (1 + alpha)])
        # model.compile(optimizer=opt, loss= ['mae',TVdist], loss_weights=[1.0, alpha])
    else:
        model.compile(optimizer=opt, loss=['mae'])

    model.summary()
    train_index, val_index, test_index = datasetgenerator(
        config, log_dir, args.exp_id, args.mostype)
    ...
    if args.phase == 'train':
        ...
        train_generator = DataGenerator(
            train_index, config, b_s, shuffle=True, mirror=False, mostype=args.mostype, saliency=args.saliency)
        val_generator = DataGenerator(
            val_index, config, 1, shuffle=False, mirror=False, mostype=args.mostype, saliency=args.saliency)
        if args.saliency == 'output':
        ...
        # it should modify when you train/test on patches, otherwise keep it as default 1.
        repeat = 1
        # totalrsults=[]
        for i in range(repeat):
            ...
            test_generator = DataGenerator(
                test_index, config, 1, shuffle=False, mirror=False, mostype=args.mostype, saliency=args.saliency)
